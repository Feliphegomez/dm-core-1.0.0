...
LITECOIND=${LITECOIND:-$SRCDIR/demedallocored}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/demedallocore-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/demedallocore-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/demedallocore-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
DMCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
